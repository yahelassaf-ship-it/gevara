...
// يُصدر حدث 'operations' فور قبول أي تعديل بشكل دائم، لتمكين البث الفوري
// (SSE) لبقية المستخدمين المتصلين بدل انتظارهم دورة الاستطلاع الدورية.
...
async function applyOperations(inputs, actor) {
  ...
  if (supabase) {
    try {
      ...
      _healthy = true;
      _lastError = null;
      if (accepted.length) events.emit('operations', { accepted, actor });
...
module.exports = { applyOperations, bootstrap, status, events };
