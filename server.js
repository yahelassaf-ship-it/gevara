...
// ===================== بث فوري (SSE) لإشعار بقية الأجهزة المتصلة =====================
// عند قبول أي تعديل بشكل دائم، تُبلَّغ كل الأجهزة المفتوحة فوراً كي تجلب
// التحديث خلال أجزاء من الثانية بدل انتظار دورة الاستطلاع (5 ثوانٍ). هذا
// البث مسجّل هنا قبل apiLimiter عمداً حتى لا يُحسب الاتصال الدائم/إعادة
// المحاولة التلقائية للمتصفح ضمن حد معدل طلبات الـ API.
...
syncDb.events.on('operations', ({ accepted, actor }) => {
  const sequence = accepted.at(-1)?.sequence || null;
  const payload = `data: ${JSON.stringify({ sequence, actor })}\n\n`;
